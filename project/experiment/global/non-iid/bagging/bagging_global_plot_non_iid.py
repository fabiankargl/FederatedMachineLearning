...
def main():
    ...
    plt.title('AUC per Round - Bagging (Non-IID)', fontsize=16)
    plt.xlabel("Round", fontsize=12)
    plt.ylabel("AUC", fontsize=12)
    plt.legend(title="Configuration", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(script_dir, "auc_plot_global_bagging_non_iid.png"), dpi=300)
    ...
    plt.title('F1 Score per Round - Bagging (Non-IID)', fontsize=16)
    plt.xlabel("Runde", fontsize=12)
    plt.ylabel("F1 Score", fontsize=12)
    plt.legend(title="Configuration", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(script_dir, "f1_plot_global_bagging_non_iid.png"), dpi=300)
